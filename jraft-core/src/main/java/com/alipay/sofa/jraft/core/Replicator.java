...
public class Replicator implements ThreadId.OnError {
    ...
    public static ThreadId start(final ReplicatorOptions opts, final RaftOptions raftOptions) {
        ...
        // Leader 节点在通过复制器 Replicator 与目标 Follower 节点建立连接后，需要发送一个探针请求，
    ...
    /**
     * 如果目标 Follower 节点运行异常，则不应该更新复制器 Replicator 的 Replicator#lastRpcSendTimestamp 字段，这无可厚非。
     * 如果目标 Follower 节点运行正常，但是拒绝当前的心跳请求，按照之前的总结分为以下三种原因：
     *
     ...
     * 其中只有第三种情况会在响应中携带 Follower 节点的最新 logIndex 值，此时心跳请求会触发向目标 Follower 节点发送探针请求，并在探针请求响应中更新 RPC 发送时间戳。
     * 然而，不管是上述哪种原因导致 Follower 节点拒绝响应，亦或是同意响应，复制器 Replicator 都会再次调用 Replicator#startHeartbeatTimer 方法进入下一轮心跳进程。
     */
     ...
     * @return send result.
     *
     * JRaft 在 AppendEntries 请求的设计上采用了将 LogEntry 的元数据和数据体相分离的策略，所以上述从本地加载日志数据的过程会先填充元数据，再填充数据体。
     * 实现上使用 RecyclableByteBufferList 作为数据体的载体，RecyclableByteBufferList 可以看做是一个可回收利用的 ByteBuffer 链表，实现层面借鉴了 Netty 的轻量级对象池的设计思想。
     */
